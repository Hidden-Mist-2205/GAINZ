...
app.put('/putFavoriteWorkout', authenticateToken, async (req, res) => {
  try {
    await controllers.toggleFavoritedWorkout(req.auth.workoutId, req.auth.userId);
...
app.put('/putFavoriteExercise', authenticateToken, async (req, res) => {
  try {
    await controllers.toggleFavoritedExercise(req.auth.exerciseId, req.auth.userId);
    res.status(204).send('Favorited');
  } catch (error) {
    console.error(error);
    res.status(500).send('Error updating data');
